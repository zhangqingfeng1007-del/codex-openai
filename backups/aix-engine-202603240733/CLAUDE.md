...
**产品库安全约束（对接阿里云只读库）：**
- 产品库凭证使用 `PRODUCT_DB_*` 环境变量，与写库 `MYSQL_*` 完全隔离
- `product_search.py` 是**唯一**允许连接产品库的模块，连接参数由 `_get_product_db_params()` 统一管理
- 进入 LLM prompt 的字段受 `_PROMPT_ALLOWED_FIELDS` 白名单约束（产品名、公司名、条款摘要等），禁止注入：`product_id`（内部标识）、`rate`（原始费率数字）、原始病名列表
- `fetch_products_text()` → 结构化文本，注入系统提示词；`fetch_products_for_scoring()` → 结构化 dict，仅供后端评分，不经过 LLM
...
- **Logo 图片：** `frontend/public/aixlogo.png`（白底/透明底），通过 `Logo.tsx` 统一渲染，高度默认 32px 等比缩放，不使用文字版本
- **collation 问题：** `cmb_product.product_id` = `utf8mb4_general_ci`，其他表 = `utf8mb4_unicode_ci`，JOIN 必须加 `COLLATE utf8mb4_unicode_ci`
- **费率存储：** `cmb_product_rate.rate` 为每千元保额的年保费（amount=1000），50万保额年保费 = rate × 500
