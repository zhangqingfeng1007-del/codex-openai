...
| 2~4 | RecommendationCard | 3 | 每产品：排名徽章 + 亮点标签 + 保费 + 性价比 + 评分条 |
...
| 模块 | 内容 |
|------|------|
| 前端入口 | ChatPage 欢迎态/对话态双布局，品牌区（AIx蓝色文字），首页快捷建议（自然问题式） |
...
| 推荐卡片组 | ProductRecommendCard（由 `product_recommendations` SSE 事件触发，第 2~7 张：RecommendationCard×3 + ProductCompareCard + RecommendationConclusionCard + RiskDisclosureCard）|
...
- [ ] 7 张展示卡片全部渲染：NeedsReportCard + RecommendationCard×3 + ProductCompareCard + RecommendationConclusionCard + RiskDisclosureCard
